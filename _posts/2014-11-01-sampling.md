...
* P-step. Given the relation $$p(\theta\mid x)=\int p(\theta\mid z,x)p(z\mid x)dz$$ we use the samples $$z(l)$$ obtained from the I-step to compute a revised estimate of the posterior distribution over θ given by $$p(\theta \mid x)=1/L\sum_l{p(\theta\mid z_l,x)}$$.
